...
public class LogModelUtil {
    ...
    private static LogModelUtil instance;
    private LogModelUtil() {
        String path = CheckDisk.checkState();
        initFile(new File(path+"logger.txt"));
    }
    public static LogModelUtil getInstance() {
        if (instance == null) {
            instance = new LogModelUtil();
        }
        return instance;
    }
    /**
